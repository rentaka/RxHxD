...
//                    受けるかどうか設定可能に。
//
// 2.2.4 2016/10/10 ・マップ描画時に自動的にプラグインコマンド[pss2 start]を
 ...
 * 簡単な使い方説明:
 *   探索者にしたいイベントのメモ欄を設定し、
 *   探索者がいるマップでプラグインコマンド pss2 start を実行すると、
 *   そのマップにいる全ての探索者が探索を開始します。
 *   (探索一時無効状態になっている探索者を除く)
 *
 *   探索者のイベント内でプラグインコマンド pss2 t_start を実行すると、
 *   その探索者が探索を開始します。
 *   (探索一時無効状態となっている探索者に対しても探索を開始させます。)
 *
 *   探索者のイベント内でプラグインコマンド pss2 t_stop を実行すると、
 *   その探索者が探索を停止します。
 *
 *   探索者がいるマップでプラグインコマンド  pss2 stop を実行すると、
 ...
 *
 *     ・この状態の場合、後述するプラグインコマンド:pss2 start実行時点では
 *       探索が開始されず、
 *         プラグインコマンド:pss2 t_start か
 *         スクリプトコマンド:$gameSystem.onsensor2(eventId) で
 *       個別に探索を開始させる必要があります。
 *
 *
 ...
 *       探索開始状態にします。
 *
 *     ・実際に探索を行わせるためには事前にpss2 startコマンドの
 *       実行が必要です。
 *
 *   pss2 t_stop
 ...
 *
 * スクリプトコマンド:
 *   $gameSystem.getEventsensor2Status(eventId)
 ...
 *           1    | 探索実行状態
 *
 *   $gameSystem.onsensor2(eventId)
 *     ・指定したイベントIDを持つ探索者を探索開始状態にします。
 *       探索停止/一時無効状態の探索者に対し探索を再開させる場合に使用します。
 *
 *     ・探索を開始させるためには事前にpss2 startコマンドの実行が必要です。
 *
 *   $gameSystem.offsensor2(eventId)
 *     ・指定したイベントIDを持つ探索者を探索停止状態にします。
 *
 *   $gameSystem.neutralsensor2(eventId, ["X","Y",...])
...
(function () {
    ...
    const2 PN = "MKR_Playersensor2";

    const2 CheckParam = function(type, name, value, def, min, max, options) {
        let regExp;
        ...
        if(!regExp.test(value)) {
            switch(type) {
                ...
                case "switch":
                    ...
                    if(name == "Lost_sensor2_Switch" && (value == null || value == undefined)) {
                        value = "";
                    }
                    if(name != "Lost_sensor2_Switch" && !value.match(/^([A-D]|\d+)$/i)) {
    ...
    const2 convertEscapeCharacters = function(text) {
        let windowChild;
    ...
    const2 ConvSw = function(text, target) {
        let num, key, regExp;
        regExp = /^\x1bV\[\d+\]$|^\x1bS\[\d+\]$/i;
    ...
    const2 paramParse = function(obj) {
        return JSON.parse(JSON.stringify(obj, paramReplace));
    };

    const2 paramReplace = function(key, value) {
        try {
    ...
    const2 Parameters = paramParse(PluginManager.parameters(PN));
    let DIR_UP, DIR_DOWN, DIR_RIGHT, DIR_LEFT,
        Defsensor2Switch, DefBothsensor2, DefRangeVisible,
        DefTerrainDecision, DefRangeColor, DefRangeOpacity,
        DefAutosensor2, DefEventDecision, DefRegionDecisions,
        DefRealRangeX, DefRealRangeY, DefLostsensor2Switch,
        DefFoundBallon, DefFoundCommon, DefFoundDelay, DefFoundSe,
        DefLostBallon, DefLostCommon, DefLostDelay, DefLostSe;
    Defsensor2Switch = CheckParam("switch", "sensor2_Switch", Parameters["sensor2_Switch"], "D");
    DefLostsensor2Switch = CheckParam("switch", "Lost_sensor2_Switch", Parameters["Lost_sensor2_Switch"]);
    DefBothsensor2 = CheckParam("bool", "Both_sensor2", Parameters["Both_sensor2"], false);
    ...
    DefAutosensor2 = CheckParam("bool", "Auto_sensor2", Parameters["Auto_sensor2"], false);
    ...
    //  を定義
    //=========================================================================
    const2 _Game_Interpreter_pluginCommand = Game_Interpreter.prototype.pluginCommand;
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
        _Game_Interpreter_pluginCommand.call(this, command, args);
        if (command.toLowerCase() === "pss2") {
            switch (args[0].toLowerCase()) {
                case "start":// 探索開始
                    $gameSystem.startsensor2();
                    break;
                case "stop":// 探索停止
                    $gameSystem.stopsensor2();
                    break;
                case "reset":// 全探索者のセルフスイッチ初期化
                    $gameSystem.resetsensor2(args);
                    break;
                case "t_start":// 対象探索者を探索開始状態にする
                    $gameSystem.onsensor2(this.eventId());
                    break;
                case "t_stop":// 対象探索者を探索停止状態にする
                    $gameSystem.offsensor2(this.eventId());
                    break;
                case "t_reset":// 対象探索者のセルフスイッチ初期化
                    $gameSystem.neutralsensor2(this.eventId(), args);
                    break;
    ...
    //  プレイヤー探索制御を定義
    //=========================================================================
    const2 _Game_System_initialize = Game_System.prototype.initialize;
    Game_System.prototype.initialize = function(){
        _Game_System_initialize.call(this);
        this._sensor2Start = false
        this._switchStatuses  = {};
    };

    Game_System.prototype.startsensor2 = function() {
        this.setsensor2Start(true);
        this.setsensor2StatusAll(1);
        this.setViewRangeStatusAll(2);
    };

    Game_System.prototype.stopsensor2 = function() {
        this.setsensor2Start(false);
        this.setsensor2StatusAll(0);
        this.setViewRangeStatusAll(0);
    };

    Game_System.prototype.resetsensor2 = function(args) {
        $gameMap.events().forEach(function(event) {
            if(event.getsensor2Type() != null) {
                $gameSystem.neutralsensor2(event.eventId(), args)
            }
        }, this);
    };

    Game_System.prototype.onsensor2 = function(eventId) {
        let event = $gameMap.event(eventId);

        if(event && event.getsensor2Type() != null) {
            event.setsensor2Status(1);
        }
    };

    Game_System.prototype.offsensor2 = function(eventId) {
        let event = $gameMap.event(eventId);

        if(event && event.getsensor2Type() != null) {
            event.setsensor2Status(0);
        }
    };

    Game_System.prototype.neutralsensor2 = function(eventId, args) {
        let mapId, event, sw, switches, sensor2Switch;
        mapId = $gameMap.mapId();
        ...
        sensor2Switch = Defsensor2Switch[0];

        if(event) {
            if(event.getsensor2Type() != null) {
                sw = (event.getsensor2Switch() != null)? event.getsensor2Switch() : sensor2Switch;
                switches.push(sw);
    ...
    Game_System.prototype.issensor2Start = function() {
        return this._sensor2Start;
    };

    Game_System.prototype.setsensor2Start = function(sensor2Start) {
        this._sensor2Start = sensor2Start || false;
    };

    Game_System.prototype.getsensor2Start = function() {
        return this._sensor2Start;
    };

    Game_System.prototype.setsensor2StatusAll = function(status) {
        $gameMap.events().forEach(function(event) {
            if(event.getsensor2Type() != null && event.getsensor2Status() != -1) {
                event.setsensor2Status(status);
            }
        }, this);
    }
    ...
    Game_System.prototype.setViewRangeStatusAll = function(status) {
        $gameMap.events().forEach(function(event) {
            if(event.getsensor2Type() != null) event.setViewRangeStatus(status);
        }, this);
    }

    Game_System.prototype.getEventsensor2Status = function(eventId) {
        let event;
        if(eventId && isFinite(eventId) && $gameMap.event(eventId)) {
            event = $gameMap.event(eventId);
            return event.getsensor2Status();
        } else {
            return null;
        }
    };
    ...
    Game_System.prototype.isFoundPlayer = function() {
        if(!this.issensor2Start()) return false;

        return $gameMap.events().some(function(event) {
            return event.getsensor2Status() == 1 && event.getFoundStatus() == 1;
        });
    };
    ...
    //  プレイヤー探索制御を定義
    //=========================================================================
    // const2 _Game_Map_setupStartingEvent = Game_Map.prototype.setupStartingEvent;
    ...
    //  センサー状態：
    //   -2 = センサー初期化前
    //   -1 = 探索停止(setsensor2StatusAllの対象外)
    //    0 = 探索停止
    //    1 = 探索中
    //  視界描画状態：
    //    0 = 描画停止
    ...
    //    1 = 発見済み
    //=========================================================================
    const2 _Game_CharacterBaseInitMembers = Game_CharacterBase.prototype.initMembers;
    Game_CharacterBase.prototype.initMembers = function() {
        ...
        this._foundStatus = 0;
        this._sensor2Status = -2;
        this._sensor2Type = null;
        this._sensor2Range = 0;
        this._sensor2RangeC = 0;
        this._bothsensor2R = false;
        this._bothsensor2L = false;
        this._viewRangeStatus = 0;
        this._coordinate = [];
        this._sensor2Switch = null;
        this._lostsensor2Switch = null;
        this._sidesensor2 = -1;
    ...
    const2 _Game_CharacterBaseMoveStraight = Game_CharacterBase.prototype.moveStraight;
    Game_CharacterBase.prototype.moveStraight = function(d) {
        let status = (this.direction() == d) ? 1 : 2;
        _Game_CharacterBaseMoveStraight.call(this,d);
        if (this.isMovementSucceeded() && d && this.getsensor2Status() == 1) {
            this.setViewRangeStatus(status);
        }
    };

    const2 _Game_CharacterBaseMoveDiagonally = Game_CharacterBase.prototype.moveDiagonally;
    Game_CharacterBase.prototype.moveDiagonally = function(horz, vert) {
        _Game_CharacterBaseMoveDiagonally.call(this,horz, vert);
        if (this.isMovementSucceeded() && this.getsensor2Status() == 1) {
            this.setViewRangeStatus(2);
        }
    };

    const2 _Game_CharacterBaseSetDirection = Game_CharacterBase.prototype.setDirection;
    Game_CharacterBase.prototype.setDirection = function(d) {
        let status = (this.direction() == d) ? 1 : 2;
        if (!this.isDirectionFixed() && d && this.getsensor2Status() == 1) {
            this.setViewRangeStatus(status);
        }
    ...
    Game_CharacterBase.prototype.setsensor2Status = function(sensor2Status) {
        this._sensor2Status = sensor2Status;
    };

    Game_CharacterBase.prototype.getsensor2Status = function() {
        return this._sensor2Status;
    };
    ...
    Game_CharacterBase.prototype.setsensor2Type = function(sensor2Type) {
        this._sensor2Type = sensor2Type;
    };

    Game_CharacterBase.prototype.getsensor2Type = function() {
        return this._sensor2Type;
    };

    Game_CharacterBase.prototype.setsensor2Range = function(sensor2Range) {
        this._sensor2Range = sensor2Range;
    };

    Game_CharacterBase.prototype.getsensor2Range = function() {
        return parseInt(ConvVb(this._sensor2Range), 10);
    };

    Game_CharacterBase.prototype.setsensor2RangeC = function(sensor2RangeC) {
        this._sensor2RangeC = sensor2RangeC;
    };

    Game_CharacterBase.prototype.getsensor2RangeC = function() {
        return parseInt(ConvVb(this._sensor2RangeC), 10);
    };
    ...
    Game_CharacterBase.prototype.setBothsensor2Right = function(bothsensor2) {
        this._bothsensor2R = bothsensor2;
    };

    Game_CharacterBase.prototype.getBothsensor2Right = function() {
        return this._bothsensor2R;
    };

    Game_CharacterBase.prototype.setBothsensor2Left = function(bothsensor2) {
        this._bothsensor2L = bothsensor2;
    };

    Game_CharacterBase.prototype.getBothsensor2Left = function() {
        return this._bothsensor2L;
    };

    Game_CharacterBase.prototype.setBothsensor2 = function(bothsensor2) {
        this._sidesensor2 = bothsensor2;
    };

    Game_CharacterBase.prototype.getBothsensor2 = function() {
        return parseInt(ConvSw(this._sidesensor2, this), 10);
    };

    Game_CharacterBase.prototype.setsensor2Switch = function(sensor2Switch) {
        if(isFinite(sensor2Switch)) {
            this._sensor2Switch = parseInt(sensor2Switch, 10);
        } else if(sensor2Switch.toLowerCase().match(/[a-d]/)) {
            this._sensor2Switch = sensor2Switch.toUpperCase();
        }
    };

    Game_CharacterBase.prototype.getsensor2Switch = function() {
        return this._sensor2Switch;
    };

    Game_CharacterBase.prototype.setLostsensor2Switch = function(sensor2Switch) {
        if(isFinite(sensor2Switch)) {
            this._lostsensor2Switch = parseInt(sensor2Switch, 10);
        } else if(sensor2Switch.toLowerCase().match(/[a-d]/)) {
            this._lostsensor2Switch = sensor2Switch.toUpperCase();
        }
    };

    Game_CharacterBase.prototype.getLostsensor2Switch = function() {
        return this._lostsensor2Switch;
    };
    ...
    //=========================================================================
    if(DefAutosensor2[0]) {
        const2 _Game_Map_setupEvents = Game_Map.prototype.setupEvents;
    ...
    //=========================================================================
    const2 _Game_EventSetupPageSettings = Game_Event.prototype.setupPageSettings;
    Game_Event.prototype.setupPageSettings = function() {
        _Game_EventSetupPageSettings.call(this);
        if(this.getsensor2Status() == -2) {
            this.setupsensor2();
        }
    ...
    Game_Event.prototype.setupsensor2 = function() {
        let event, pattern, match, note, cnt, i, n, m,
            options, op, value;
        event = this.event();
        pattern = /<(.?)(?:psensor2)(l|f|s|d)?(?:\:)(\\v\[\d+\]|\d+)([ 0-9a-z\[\]\\]*)?>/i

        if(note) {
            note = event.Psensor2D:8.toLowerCase();
            note = Psensor2D:8.split(/ (?=<)/);
            cnt = note.length;
            ...
            for(i = 0;i < cnt;i++) {
                ...
                if(n.match(pattern)) {
                    ...
                    if(match[3]) { // 探索対象マス数
                        ...
                        value = value.replace(/\\/g, '\x1b');
                        value = value.replace(/\x1b\x1b/g, '\\');
                        this.setsensor2Range(match[3]);
                        this.setsensor2RangeC(match[3]);
                    }
                    if(match[4]) { // オプション
    ...
    const2 _Game_EventUpdate = Game_Event.prototype.update;
    Game_Event.prototype.update = function() {
        _Game_EventUpdate.call(this);
        if($gameSystem.issensor2Start()) this.sensor2Update();
    };

    Game_Event.prototype.sensor2Update = function() {
        // 探索中のイベントであること
        if(this.getsensor2Status() == 1){
    ...
    Game_Event.prototype.foundPlayer = function() {
        let mapId, eventId, sw, key, sensor2Switch, delay, lostsensor2Switch;
        delay = this.getFoundDelay();

        if(delay <= 0) {
            sensor2Switch = Defsensor2Switch[0];
            lostsensor2Switch = DefLostsensor2Switch[0];
            mapId = $gameMap.mapId();
            ...
            // 発見後スイッチON
            sw = (this.getsensor2Switch() != null)? this.getsensor2Switch() : sensor2Switch;
            if(isFinite(sw)) {
            ...
            // ロスト後スイッチOFF
            sw = (this.getLostsensor2Switch() != null)? this.getLostsensor2Switch() : lostsensor2Switch;
            if(sw != "") {
                if(isFinite(sw)) {
                    if($gameSwitches.value(sw)) {
                        $gameSwitches.setValue(sw, false);
    ...
    Game_Event.prototype.isFoundPlayer = function() {
        let result = false;
        switch(this.getsensor2Type()) {
            case "l": // 直線の探索
                result = this.sensor2Line();
                break;
            case "f": // 扇範囲の探索
                result = this.sensor2Fan();
                break;
            case "s": // 四角範囲の探索
                result = this.sensor2Square();
                break;
            case "d": // 菱形範囲の探索
                result = this.sensor2Diamond();
                break;
        }
        // if(result) {
    ...
    // 直線の探索
    Game_Event.prototype.sensor2Line = function() {
        let sensor2Range, sensor2RangeC, strDir, diagoDir, dir, dirFixed, ex, ey, i,
        coordinates, px, py, cnt, realX, realY;
        sensor2Range = this.getsensor2Range();
        ...
        // currentRange初期化
        //this.setsensor2RangeC(sensor2Range);
        sensor2RangeC = sensor2Range;
        ...
        switch(dir) {
            case 8:// 上向き(y<0)
                ...
                // 正面範囲確定
                this.rangeSearch(strDir, 0, 0, 0, -1, sensor2Range);
            ...
            case 6:// 右向き(x>0)
                ...
                // 正面範囲確定
                this.rangeSearch(strDir, 0, 0, 1, 0, sensor2Range);
    ...
    // 扇範囲の探索
    Game_Event.prototype.sensor2Fan = function() {
        let sensor2Range, sensor2RangeC, dir, dirFixed, sx, sy, ex, ey, px, py,
        ...
        switch(dir) {
            case DIR_UP:// 上向き(y<0)
                ...
                // 正面範囲確定
                noPass = this.rangeSearch(strDir, 0, 0, 0, -1, sensor2Range);
                if(noPass != sensor2Range) noPass++;
                ...
                for(i = 1;i < 3; i++) {
                    ...
                    // 配列の要素数合わせ
                    this.addCoordinate(sensor2Range * i + 1 + i);
            ...
            case DIR_RIGHT:// 右向き(x>0)
                ...
                // 正面範囲確定
                noPass = this.rangeSearch(strDir, 0, 0, 1, 0, sensor2Range);
                if(noPass != sensor2Range) noPass++;
    ...
    // 菱形範囲の探索(地形考慮完全無視)
    Game_Event.prototype.sensor2Diamond = function() {
        let sensor2Range, sx, sy, realX, realY;
        sensor2Range = this.getsensor2Range();
        ...
        // coordinateセット
        this.setCoordinate(0, -sensor2Range, DIR_RIGHT);
        this.setCoordinate(sensor2Range, 0, DIR_DOWN);
        this.setCoordinate(0, sensor2Range, DIR_LEFT);
        this.setCoordinate(-sensor2Range, 0, DIR_UP);

        // プレイヤー範囲探索
        if(Math.abs(sx) + Math.abs(sy) <= sensor2Range + Math.max(realX, realY)) {
            return true;
        }
    }

    // 四角範囲の探索(地形考慮完全無視)
    Game_Event.prototype.sensor2Square = function() {
        let sensor2Range, sx, sy, realX, realY;
        sensor2Range = this.getsensor2Range();
        sx = this.deltaXFrom($gamePlayer._realX);
        sy = this.deltaYFrom($gamePlayer._realY);
        ...
        // プレイヤー範囲探索
        if(Math.abs(sx) <= sensor2Range + realX && Math.abs(sy) <= sensor2Range + realY) {
            return true;
        }
    }

    Game_Event.prototype.isSideSearch = function(directionR, directionL, vx, vy) {
        let sx, sy, ex, ey, bothsensor2, terrainDecision, realX, realY;
        sx = this.deltaXFrom($gamePlayer._realX);
        sy = this.deltaYFrom($gamePlayer._realY);
        ex = this.x;
        ey = this.y;
        bothsensor2 = CEC(DefBothsensor2);
        terrainDecision = CEC(DefTerrainDecision);
        realX = DefRealRangeX[0];
        realY = DefRealRangeY[0];

        if(this.getBothsensor2() == -1 && bothsensor2) {
            if(this.getTerrainDecision() == 1
                    || (this.getTerrainDecision() == -1 && terrainDecision)) {
                this.setBothsensor2Right(this.isMapPassableEx(ex, ey, directionR));
                this.setBothsensor2Left(this.isMapPassableEx(ex, ey, directionL));
            } else {
                this.setBothsensor2Right(true);
                this.setBothsensor2Left(true);
            }
        } else if(this.getBothsensor2() == 1) {
            if(this.getTerrainDecision() == 1
                    || (this.getTerrainDecision() == -1 && terrainDecision)) {
                this.setBothsensor2Right(this.isMapPassableEx(ex, ey, directionR));
                this.setBothsensor2Left(this.isMapPassableEx(ex, ey, directionL));
            ...
            this.setBothsensor2Right(false);
            this.setBothsensor2Left(false);
        }

        if(this.getBothsensor2Right()
    ...
    Game_Event.prototype.rangeSearch = function(strDir, rx, ry, signX, signY, noPass) {
        let sensor2Range, ex, ey, cx, cy, sx, sy, j, obstacle ,cnt, status,
            noPassDir, terrainDecision;
        sensor2Range = this.getsensor2Range();
        cnt = sensor2Range - Math.abs(rx);
        ex = this.x;
        ey = this.y;
        obstacle = -1;
        status = "Last";
        ...
        for(j = 0; j <= cnt; j++) {
            ...
            if(this.getTerrainDecision() == 1
                    || (this.getTerrainDecision() == -1 && terrainDecision)) {
                if(!this.isMapPassableEx(ex + cx, ey + cy, strDir) && j < sensor2Range) {
                    obstacle = j + Math.abs(rx);
                    status = "Line";
                    break;
                }
                if(j + Math.abs(noPassDir) >= noPass && noPass < sensor2Range) {
                    status = "Nopass";
                    break;
                }
            }
    ...
    const2 _GameEvent_lock = Game_Event.prototype.lock;
    ...
    //=========================================================================
    const2 _Spriteset_Map_createLowerLayer = Spriteset_Map.prototype.createLowerLayer;
    ...
    const2 _Spriteset_Base_update = Spriteset_Base.prototype.update;
    ...
    Sprite_ViewRange.prototype.const2ructor = Sprite_ViewRange;
    ...
    Sprite_ViewRange.prototype.update = function() {
        let sensor2Status, rangeStatus, rangeVisible, defVisible;

        Sprite.prototype.update.call(this);

        sensor2Status = this._character.getsensor2Status();
        rangeStatus = this._character.getViewRangeStatus();
        rangeVisible = this._character.getRangeVisible();
        defVisible = CEC(DefRangeVisible);

        if(sensor2Status == 1 && (rangeVisible == 1 || (rangeVisible == -1 && defVisible))) {
            this.updatePosition();
            if(this.bitmap) {
                if(rangeStatus == 1) {
                    // 描画更新
    ...
    Sprite_ViewRange.prototype.createBitmap = function() {
        let direction, dirFixed, sensor2Range, sensor2RangeC, sensor2Type,
            tileWidth, tileHeight, width, height, coordinates,
            sidesensor2R, sidesensor2L, bs, bias, color, opacity,
            bothsensor2;
        ...
        sensor2Type = this._character.getsensor2Type();
        sensor2RangeC = this._character.getsensor2RangeC();
        sensor2Range = this._character.getsensor2Range();
        tileWidth = $gameMap.tileWidth();
        tileHeight = $gameMap.tileHeight();
        sidesensor2R = this._character.getBothsensor2Right();
        sidesensor2L = this._character.getBothsensor2Left();
        bias = (bothsensor2)? 3
            : (this._character.getBothsensor2() > 0)? 3 : 1;
        ...
        switch(sensor2Type) {
            case "l":
                if(direction == DIR_UP) {
                    ...
                    height = tileHeight * sensor2Range + tileWidth;
                    this.anchor.x = 0.5;
                    this.anchor.y = 1;
                } else if(direction == DIR_RIGHT) {
                    width = tileWidth * sensor2Range + tileWidth;
                    ...
                    width = tileWidth * bias;
                    height = tileHeight * sensor2Range + tileHeight;
                    this.anchor.x = 0.5;
                    this.anchor.y = 0;
                }
            ...
            case "f":
                if(direction == DIR_UP) {
                    width = tileWidth * sensor2Range * 2 + tileWidth;
                    height = tileHeight * sensor2Range + tileWidth;
                    this.anchor.x = 0.5;
                    this.anchor.y = 1;
                } else if(direction == DIR_RIGHT) {
                    width = tileWidth * sensor2Range + tileWidth;
                    height = tileHeight * sensor2Range * 2 + tileHeight;
                    this.anchor.x = 0;
                    this.anchor.y = 0.5;
                } else if(direction == DIR_LEFT) {
                    width = tileWidth * sensor2Range + tileWidth;
    ...
    Sprite_ViewRange.prototype.updatePosition = function() {
        let direction, dirFixed, sensor2RangeC, sensor2Type, cx, cy, tileWidth, tileHeight, bias;
        direction = this._character.direction();
        dirFixed = this._character.getDirectionFixed();
        direction = (dirFixed == -1)? direction : dirFixed;
        sensor2Type = this._character.getsensor2Type();
    ...
    Bitmap.prototype.fillViewRangeLine = function(color, character) {
        let context, width, height, tileWidth, tileHeight,
            ...
            direction, dirFixed, coordinates, sidesensor2R, sidesensor2L;
        ...
        cnt = coordinates.length;
        sidesensor2R = character.getBothsensor2Right();
        sidesensor2L = character.getBothsensor2Left();
        j = 0;
        ...
        if(direction == DIR_UP) {
            if(coordinates && cnt == 1) {
                ...
                this.mkrSideDrawLine(context, cx, cy, [sidesensor2R, 1, 1, sidesensor2L, -1, 1]);
    ...
    Bitmap.prototype.fillViewRangeFan = function(color, character) {
        let context, width, height, tileWidth, tileHeight, cx, cy,
            coordinates, direction, dirFixed, sidesensor2R, sidesensor2L,
    ...
    Bitmap.prototype.mkrSideDrawLine = function(context, cx, cy, sidesensor2s) {
        let tileWidth, tileHeight, rx, ry, signX, signY, signX2, signY2;
        tileWidth = $gameMap.tileWidth();
        tileHeight = $gameMap.tileHeight();
        signX = sidesensor2s[1];
        signY = sidesensor2s[2];
        signX2 = sidesensor2s[4];
        signY2 = sidesensor2s[5];
